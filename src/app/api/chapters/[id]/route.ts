...
export async function GET(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const chapter = await getChapterDetail(getDatabase(), auth.user.id, id, projectId);
...
export async function PATCH(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    const projectId = new URL(request.url).searchParams.get("projectId") ?? undefined;
    const chapter = await updateChapterPlan(getDatabase(), auth.user.id, id, projectId, parsed.data);
    if (!chapter) return NextResponse.json({ code: "CHAPTER_NOT_FOUND", error: "章节不存在。" }, { status: 404 });
    return NextResponse.json({ chapter });
  } catch (error) {
