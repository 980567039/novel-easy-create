...
export default function AiSettingsPage() {
  ...
  if (loading) return <main className="flex min-h-[100dvh] items-center justify-center px-4 text-slate-500"><Loader2 className="mr-2 animate-spin" size={18} />正在加载设置...</main>;
  ...
  return (
    <main className="min-h-[100dvh] bg-slate-50 pb-[max(1.5rem,env(safe-area-inset-bottom))] pl-[max(1rem,env(safe-area-inset-left))] pr-[max(1rem,env(safe-area-inset-right))] pt-[max(1.5rem,env(safe-area-inset-top))] text-slate-900 sm:pb-[max(2rem,env(safe-area-inset-bottom))] sm:pl-[max(2rem,env(safe-area-inset-left))] sm:pr-[max(2rem,env(safe-area-inset-right))] sm:pt-[max(2rem,env(safe-area-inset-top))]">
      <div className="mx-auto max-w-3xl">
        <button type="button" onClick={() => router.push("/")} className="mb-6 inline-flex min-h-11 items-center gap-2 text-sm font-medium text-slate-500 hover:text-indigo-600 sm:mb-8"><ArrowLeft size={16} /> 返回首页</button>
        <div className="mb-6 flex items-start gap-3 sm:mb-8 sm:gap-4"><div className="shrink-0 rounded-2xl bg-indigo-100 p-2.5 text-indigo-600 sm:p-3"><Server size={24} className="sm:h-[26px] sm:w-[26px]" /></div><div className="min-w-0"><p className="mb-1 text-sm font-semibold tracking-wide text-indigo-600">模型设置</p><h1 className="text-2xl font-extrabold tracking-tight sm:text-4xl">连接你的 AI</h1><p className="mt-2 text-slate-500">小说分析、故事规划和章节生成都会使用这里配置的模型。</p></div></div>
        <form onSubmit={save} className="space-y-6 rounded-3xl border border-slate-200 bg-white p-5 shadow-sm sm:p-9">
          ...
          <div className="grid grid-cols-2 gap-2 border-t border-slate-100 pt-5 sm:flex sm:flex-wrap sm:justify-end sm:gap-3"><button type="button" onClick={testConnection} disabled={testing} className="inline-flex min-h-11 items-center justify-center gap-2 rounded-xl border border-slate-200 px-3 text-sm font-semibold text-slate-700 hover:bg-slate-50 disabled:opacity-50 sm:px-4"><Wifi size={17} />{testing ? "测试中..." : "测试连接"}</button><button type="submit" disabled={saving} className="inline-flex min-h-11 items-center justify-center gap-2 rounded-xl bg-indigo-600 px-3 text-sm font-semibold text-white shadow-sm hover:bg-indigo-700 disabled:opacity-50 sm:px-4"><Save size={17} />{saving ? "保存中..." : "保存设置"}</button></div>
        </form>
      </div>
    </main>
  );
