...
function settingsKey(readerKey: string) {
  return `novel-role:reader-settings:${readerKey}`;
}

function loadSettings(readerKey: string): ReaderSettings {
  try {
    const raw = window.localStorage.getItem(settingsKey(readerKey));
...
export function NovelReader({ endpoint, readerKey, projectId, publicView = false }: NovelReaderProps) {
  const scrollRef = useRef<HTMLElement | null>(null);
  const privateProjectId = publicView ? undefined : projectId;
  ...
  const requestReader = useCallback((chapterId?: string, signal?: AbortSignal) => {
    const url = chapterId ? `${endpoint}?chapterId=${encodeURIComponent(chapterId)}` : endpoint;
    return apiFetch(
      url,
      { cache: "no-store", ...(signal ? { signal } : {}) },
      { handleUnauthorized: !publicView },
    );
  }, [endpoint, publicView]);
  ...
  const loadDirectory = useCallback(async () => {
    ...
    try {
      ...
      setProject(normalizeProject(body.project, readerKey));
      setStats(body.stats ?? {});
  ...
  }, [readerKey, requestReader]);

  useEffect(() => {
    setSettings(loadSettings(readerKey));
    setSettingsReady(true);
    void loadDirectory();
  }, [loadDirectory, readerKey]);

  useEffect(() => {
    if (!settingsReady) return;
    window.localStorage.setItem(settingsKey(readerKey), JSON.stringify(settings));
  }, [readerKey, settings, settingsReady]);
  ...
  useEffect(() => {
    ...
    async function loadChapter() {
      ...
      setChapter(null);
      try {
        const response = await requestReader(selectedId ?? "", controller.signal);
        ...
        if (body.project) setProject(normalizeProject(body.project, readerKey));
  ...
  }, [chapterReloadToken, readerKey, requestReader, selectedId, selectedSummary]);
    ...
    return (
      <main className="flex h-[100dvh] items-center justify-center bg-[#f2efe8] px-6 text-slate-700">
        <div className="max-w-sm text-center">
          ...
          <h1 className="mt-4 text-lg font-semibold">{publicView ? "分享链接不可用" : "小说暂时打不开"}</h1>
          <p className="mt-2 text-sm leading-6 text-slate-500">{directoryError}</p>
          <div className="mt-5 flex justify-center gap-3">
            {privateProjectId && <Link href={`/projects/${privateProjectId}`} className="rounded-xl border border-stone-300 px-4 py-2 text-sm">回工作台</Link>}
  ...
  return (
    ...
    >
      ...
      >
        <div className="flex h-14 items-center gap-2 pl-[max(0.75rem,env(safe-area-inset-left))] pr-[max(0.75rem,env(safe-area-inset-right))] sm:h-16 sm:pl-[max(1.5rem,env(safe-area-inset-left))] sm:pr-[max(1.5rem,env(safe-area-inset-right))]">
          {privateProjectId ? (
            <Link
              href={`/projects/${privateProjectId}`}
              aria-label="返回项目工作台"
            ...
            <span aria-label="小白作家公开阅读" className="flex h-11 w-11 shrink-0 items-center justify-center rounded-xl">
          ...
          {privateProjectId && selectedSummary && (
            <Link
              href={`/projects/${privateProjectId}/chapters?chapterId=${encodeURIComponent(selectedSummary.id)}`}
      ...
      >
        <article className={`mx-auto min-h-full w-full max-w-[800px] py-8 pl-[max(1rem,env(safe-area-inset-left))] pr-[max(1rem,env(safe-area-inset-right))] transition-colors sm:my-8 sm:min-h-[calc(100%-4rem)] sm:rounded-2xl sm:px-16 sm:py-16 sm:shadow-xl lg:px-20 ${colors.article}`}>
          {loadingChapter ? (
            ...
            <>
              <div className="mb-10 border-b border-current/10 pb-8 text-center">
                ...
                <div className={`mt-4 flex flex-wrap items-center justify-center gap-x-3 gap-y-1 text-xs ${colors.muted}`}>
                  ...
                  <span>{publicView ? "已发布" : statusLabel(chapter ?? selectedSummary)}</span>
            ...
            <div className="flex min-h-[60vh] flex-col items-center justify-center text-center">
              ...
              <h1 className="mt-5 text-xl font-semibold">{publicView ? "暂时没有可阅读正文" : "这一章还没有正文"}</h1>
              <p className={`mt-2 max-w-sm text-sm leading-6 ${colors.muted}`}>{publicView ? "作者暂未通过这个链接公开章节正文，请稍后再来看看。" : "这里只展示已保存的小说正文，不会用大纲摘要代替。完成创作并保存草稿后，就可以在这里预览。"}</p>
              {privateProjectId && selectedSummary && (
                <Link href={`/projects/${privateProjectId}/chapters?chapterId=${encodeURIComponent(selectedSummary.id)}`} className="mt-6 inline-flex min-h-11 items-center gap-2 rounded-xl bg-indigo-600 px-5 py-2 text-sm font-medium text-white">
        ...
        <div className="fixed inset-0 z-50 flex" role="dialog" aria-modal="true" aria-label="小说目录">
          ...
          >
            ...
            <div className="min-h-0 flex-1 overflow-y-auto px-3 py-3">
              {chapters.length === 0 ? (
                <p className={`px-3 py-10 text-center text-sm ${colors.muted}`}>{publicView ? "作者暂未公开可阅读章节。" : "还没有章节，请先生成大纲。"}</p>
                    ...
                    >
                      ...
                      <span className="min-w-0 flex-1">
                        ...
                        <span className={`mt-0.5 block text-[11px] ${colors.muted}`}>
                          {item.hasContent ? `${formatWordCount(item.wordCount)} · ${publicView ? "已发布" : statusLabel(item)}` : "待创作 · 暂无正文"}
                        </span>
                      </span>
            ...
            {privateProjectId && selectedSummary && (
              <div className="shrink-0 border-t border-current/10 p-3">
                <Link href={`/projects/${privateProjectId}/chapters?chapterId=${encodeURIComponent(selectedSummary.id)}`} className={`flex min-h-11 items-center justify-center gap-2 rounded-xl border text-sm ${colors.subtle} ${colors.hover}`}>
